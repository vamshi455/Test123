...
LatestContributionFactor AS (
    SELECT
        ...
        sf.FACTOR,
        ROW_NUMBER() OVER (
            PARTITION BY sf.ID_PATTERN, sf.ID_COMPLETION
            ORDER BY sf.EFFECT_DATE DESC
        ) AS rn
...
Splits AS (
    SELECT
        ...
        COALESCE(
            ((dv.THEOR_GAS_VOL_KSCF * 1000 / NULLIF(dv.THEOR_OIL_VOL_STB, 0) - pvt.SOLUTION_GAS_OIL_RATIO) * dv.THEOR_OIL_VOL_STB) * sf.FACTOR * pvt.GAS_FORMATION_VOLUME_FACTOR,
            0
        ) AS FREE_GAS,
        sf.FACTOR,
        pp.PRESSURE,
    ...
    JOIN LATERAL RMDE_SAM_ACC.InterpolatePVTCompletionTest(
        pp.PRESSURE::FLOAT,
        dv.COMPLETION_ID::VARCHAR(32),
        pp.DATE::DATE
    ) pvt ON TRUE
),
Final AS (
